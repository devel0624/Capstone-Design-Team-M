...
actions = ['Stop', 'Play']
#, 'VolUp', 'VolDn', 'Mute','Forward', 'Rewind']
seq_length = 30

model = tf.keras.models.load_model('models/demo_model.h5')
    ...
    if result.multi_hand_landmarks is not None:
        for res in result.multi_hand_landmarks:
            ...
            if action_seq[-1] == action_seq[-2] == action_seq[-3]:
                ...
                #if last_action == 'StartMotino' and this_action != 'StartMotion' :
                
                if last_action != this_action:
                    if this_action == 'Stop':
                        pag.press('space')
                    elif this_action == 'Play':
                        pag.press('space')
                    """
