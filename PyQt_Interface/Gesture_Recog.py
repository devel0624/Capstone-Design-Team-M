import sys
import cv2
import Model_Process as MOD
from numpy import zeros
from Model_Process import mp_hands, mp_drawing
...
#seq = [],action_seq = [],last_action = None

def Gesture_Cap(cap,seq,action_seq,last_action):
    while cap.isOpened():
        ret, img = cap.read()
        if not ret:
            break

        result, img = MOD.Set_Video(img)
        ...
        if result.multi_hand_landmarks is not None:
            for res in result.multi_hand_landmarks:
                ...
                i_pred, conf = MOD.Predict(seq,seq_length)
                ...
                if action_seq[-1] == action_seq[-2] == action_seq[-3]:
                    ...
                    last_action = MOD.What_Action(this_action, last_action)
